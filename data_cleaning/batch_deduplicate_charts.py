...
def write_uniques(chunk, chunk_num, output_file):
    chunk_rows = []
    start_time = time()
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        output_file_name = '../processed/deduplicated/{}_deduplicated_one_per_user.tsv'.format(
            input_file_name.rsplit('.', 1)[0])
        output_file = csv.writer(open(output_file_name, 'w'), delimiter='\t')
        output_file.writerow(headers)
        for i, chunk in enumerate(raw_df_chunks):
            r = write_uniques(chunk, i + 1, output_file)
